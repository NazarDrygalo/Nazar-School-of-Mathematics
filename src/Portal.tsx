...
export function AdminDashboard() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase.from('applications').select('id, created_at, status, notification_status, service_area, help_areas, academic_goals, preferred_days, preferred_times, timezone, parents(first_name,last_name,email,phone), students(first_name,last_name,grade,school,current_course)').order('created_at', { ascending: false })
  ...
  return <section className="dashboard"><header className="dashboard-header"><div><p className="eyebrow">Administrator portal</p><h1>Applications</h1><p>Review incoming tutoring applications and keep their status current.</p></div><button className="text-button" onClick={signOut}>Sign out</button></header>{message && <p className="dashboard-message" role="status">{message}</p>}<div className="application-layout"><div className="application-list">{applications.length === 0 ? <p className="empty-state">No applications have been received yet.</p> : applications.map(application => <button key={application.id} className={`application-row ${selected?.id === application.id ? 'selected' : ''}`} onClick={() => setSelected(application)}><span><b>{application.students?.first_name} {application.students?.last_name}</b><small>{application.service_area} · {application.students?.grade} · {application.students?.current_course}</small></span><span className={`status status-${application.status}`}>{application.status}</span><small>{new Date(application.created_at).toLocaleDateString()}</small></button>)}</div><ApplicationDetail application={selected} onStatus={setApplicationStatus} /></div></section>
}
...
function ApplicationDetail({ application, onStatus }: { application: Application | null; onStatus: (application: Application, nextStatus: string) => void }) {
  ...
  return <aside className="application-detail"><div className="detail-top"><div><p className="eyebrow">Application detail</p><h2>{student?.first_name} {student?.last_name}</h2></div><label>Application status<select value={application.status} onChange={event => onStatus(application, event.target.value)}>{['submitted','reviewing','accepted','declined','withdrawn'].map(value => <option key={value} value={value}>{value}</option>)}</select></label></div><div className="detail-grid"><Detail label="Tutoring area">{application.service_area}</Detail><Detail label="Student">{student?.grade}<br />{student?.school}<br />{student?.current_course}</Detail><Detail label="Parent / guardian">{parent?.first_name} {parent?.last_name}<br /><a href={`mailto:${parent?.email}`}>{parent?.email}</a><br />{parent?.phone || 'No phone provided'}</Detail><Detail label="Areas needing help">{application.help_areas}</Detail><Detail label="Academic goals">{application.academic_goals}</Detail><Detail label="Availability">{application.preferred_days}<br />{application.preferred_times}<br />{application.timezone}</Detail><Detail label="Notification">{application.notification_status}</Detail></div></aside>
}
